		...
		ImmaturePointTemporaryResidual* residuals)
	{
		...
		for (FrameHessian* fh : frameHessians)
		{
			if (fh != point->host)
			{
				...
				residuals[nres].state_state = ResState::INNER;
		...
		{
			lastEnergy += point->linearizeResidual(&Hcalib, 1000, residuals + it, lastHdd, lastbd, currentIdepth);
			residuals[it].state_state = residuals[it].state_NewState;
			residuals[it].state_energy = residuals[it].state_NewEnergy;
		}
		...
		int numGoodRes = 0;
		for (int it = 0; it < nres; ++it)
			if (residuals[it].state_state == ResState::INNER) numGoodRes++;
		...
		for (int it = 0; it < nres; ++it)
		{
			if (residuals[it].state_state == ResState::INNER)
			{
				PointFrameResidual* r = new PointFrameResidual(p, p->host, residuals[it].target);
				r->state_NewEnergy = r->state_energy = 0;
				r->state_NewState = ResState::OUTLIER;
				r->setState(ResState::INNER);
				p->residuals.push_back(r);
