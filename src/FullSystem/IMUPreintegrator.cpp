...
namespace dso {
    IMUPreintegrator::IMUPreintegrator(const IMUPreintegrator &pre) :
            ...
            _J_P_Biasg(pre._J_P_Biasg), _J_P_Biasa(pre._J_P_Biasa), _J_V_Biasg(pre._J_V_Biasg),
            _J_V_Biasa(pre._J_V_Biasa), _J_R_Biasg(pre._J_R_Biasg),
    ...
    {
        _gyr_data.emplace_back(omega);
        _acc_data.emplace_back(acc);
        double dt2 = dt * dt;
        ...
        _cov_P_V_Phi = A * _cov_P_V_Phi * A.transpose() +
                       Bg * GyrCovariance * Bg.transpose() +
                       Ca * AccCovariance * Ca.transpose();
